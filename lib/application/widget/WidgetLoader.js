...
var Application_WidgetLoader = Object.extend(Object, {
	...
	_instantiateWidget: function(oElement, sWidgetName){
		...
		if (eval("typeof(Application_Widget"+sWidgetName+")") == "undefined"){
			...
			// Check here requirement before load
			if (this.aRequireWidget.length > 0){
				for (var i=0; i < this.aRequireWidget.length; i++){
					//TODO Load required JS if not loaded
					if (eval("typeof(Application_" + this.aRequireWidget[i] + ")") == "undefined"){
						// Load dependency
					}
				}
			}else{
				// Go ahead and load/instantiate
			}
